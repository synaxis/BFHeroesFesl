...
func (fm *FeslManager) NuLookupUserInfo(event network.EventClientCommand) {
	...
	for i := 0; i < keys; i++ {
		...
		var id, userID, heroName string
		err := fm.db.stmtGetHeroeByName.QueryRow(heroNamePacket).Scan(&id, &userID, &heroName)
		if err != nil {
			return
		}
...
func (fm *FeslManager) NuLoginPersonaServer(event network.EventClientCommand) {
	...
	if err != nil {
		logrus.Println("Bad Server Login")
		return
	}
